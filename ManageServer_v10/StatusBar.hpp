...
public:
	...
	)
	{
		...
		StringCchCopy(pTabItem[0]->pszTextParts[1]
			, PARTS::TEXT_MAX
			, L"Monitor ON"
		);
		StringCchCopy(pTabItem[0]->pszTextParts[2]
			, PARTS::TEXT_MAX
			, L"Server DEAD"
		);
		StringCchCopy(pTabItem[0]->pszTextParts[5]
			, PARTS::TEXT_MAX
			, L"Logfile size 0 (kb)"
		);
	}
	int SetStatusBar(const HWND& hWnd)
	{
		...
		SendMessage(hWndStatus
			, SB_SETTEXT
			, (WPARAM)1
			, (LPARAM)pTabItem[0]->pszTextParts[1]
		);
		SendMessage(hWndStatus
			, SB_SETTEXT
			, (WPARAM)2
			, (LPARAM)pTabItem[0]->pszTextParts[2]
		);
		SendMessage(hWndStatus
			, SB_SETTEXT
			, (WPARAM)5
			, (LPARAM)pTabItem[0]->pszTextParts[5]
		);
		// free the array
		LocalUnlock(hloc);
		LocalFree(hloc);
